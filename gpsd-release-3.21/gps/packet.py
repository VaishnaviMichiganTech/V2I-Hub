...
Look in, current directory, GPSD_HOME, then use find_library().
find_library() looks in: LD_LIBRARY_PATH, DYLD_LIBRARY_PATH,
$home/lib, /.usr/local/lib, /usr/lib, /lib
Returns tha library handle."""

    packet_paths = []         # places to look
    packet_name = 'libgpsdpacket.so.27.0.0'

    # look in current dir first.  To match what "import" does.
    # Make sure it is a full path
    packet_paths.append(os.path.abspath(packet_name))

    # Check GPSD_HOME environment variable, look there second
    if 'GPSD_HOME' in os.environ:
        # Make sure it is a full path
        packet_paths.append(os.path.abspath(os.environ['GPSD_HOME'] +
                                            os.sep + packet_name))
